...
lr = [0.01]
premodel='model_densenet201/model_30.pkl'
my_open = open('loss-densenet201.txt', 'a')
# save model name
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_classes', type=int, default=6,
                        help="""num classes""")
    parser.add_argument('--model_path', type=str, default='model_densenet201',
    ...
    transform = transforms.Compose([
        transforms.Resize(224),  # 将图像转化为32 * 32
    ...
    # Load data
    test_datasets = torchvision.datasets.ImageFolder(root=os.path.join(args.path, 'test'),
                                                      transform=transform)
    ...
    #model = densenet161(pretrained=False).to(device)
    model=torch.load(premodel)
